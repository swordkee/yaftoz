...
void zephir_concat_vsss(zval **result, zval *op1, const char *op2, zend_uint op2_len, const char *op3, zend_uint op3_len, const char *op4, zend_uint op4_len, int self_var TSRMLS_DC){
	...
	length = Z_STRLEN_P(op1) + op2_len + op3_len + op4_len;
	if (self_var) {
...
void zephir_concat_vsssvss(zval **result, zval *op1, const char *op2, zend_uint op2_len, const char *op3, zend_uint op3_len, const char *op4, zend_uint op4_len, zval *op5, const char *op6, zend_uint op6_len, const char *op7, zend_uint op7_len, int self_var TSRMLS_DC){
	...
	zval result_copy, op1_copy, op5_copy;
	int use_copy = 0, use_copy1 = 0, use_copy5 = 0;
	uint offset = 0, length;
	...
	length = Z_STRLEN_P(op1) + op2_len + op3_len + op4_len + Z_STRLEN_P(op5) + op6_len + op7_len;
	if (self_var) {
	...
	memcpy(Z_STRVAL_PP(result) + offset + Z_STRLEN_P(op1) + op2_len, op3, op3_len);
	memcpy(Z_STRVAL_PP(result) + offset + Z_STRLEN_P(op1) + op2_len + op3_len, op4, op4_len);
	memcpy(Z_STRVAL_PP(result) + offset + Z_STRLEN_P(op1) + op2_len + op3_len + op4_len, Z_STRVAL_P(op5), Z_STRLEN_P(op5));
	memcpy(Z_STRVAL_PP(result) + offset + Z_STRLEN_P(op1) + op2_len + op3_len + op4_len + Z_STRLEN_P(op5), op6, op6_len);
	memcpy(Z_STRVAL_PP(result) + offset + Z_STRLEN_P(op1) + op2_len + op3_len + op4_len + Z_STRLEN_P(op5) + op6_len, op7, op7_len);
	Z_STRVAL_PP(result)[length] = 0;
	Z_TYPE_PP(result) = IS_STRING;
	Z_STRLEN_PP(result) = length;
